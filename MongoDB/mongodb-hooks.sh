...
# This execution hook has been tested with MongoDB 7.0.12 (deployed using Bitnami Helm chart) and NetApp Astra Control Service 22.04
# args: [pre|post]
...
prehook() {
    ...
    mongosh admin --eval "printjson(db.fsyncLock())" --authenticationDatabase admin -u root -p $MONGODB_ROOT_PASSWORD
    return 0
}
#
...
posthook() {
    ...
    mongosh admin --eval "printjson(db.fsyncUnlock())" --authenticationDatabase admin -u root -p $MONGODB_ROOT_PASSWORD
    return 0
}
#
# main
